...
int main()
{
    ...
    int rand_num1,rand_num2,temp1;       // rand_num will be used to store the result of random number generations, temp1 will be used as a temporary variable
    ...
    do{
        ...
        // 1- generate a random main board by shuffling the main_board      (COMPLETE THIS PART)
            ...
            {									
			    rand_num1 = rand()%9;									
			    rand_num2= rand()%9;									
			    temp1 =main_board[rand_num1];							
			    main_board[rand_num1]= main_board[rand_num2];			
			    main_board[rand_num2]=temp1;							
		}
		...
		{														
			...
			{											
				row_results[i/2] = main_board[3*i/2] * main_board[3*i/2+1];		
			}
			    else if(operators_array[i]== '-')
			    {										
				    row_results[i/2] = main_board[3*i/2] - main_board[3*i/2+1];		
			    }
			        else
			...
			{						
				row_results[i/2] *= main_board[3*i/2+2];		 
			}
			    else if(operators_array[i+1]== '-')
			    {				
				    row_results[i/2] -= main_board[3*i/2+2];		
			    }
			        else
		...
		{														
			...
			if(operators_array[i]== '*')
			{											
				column_results[i-6] = main_board[i-6] * main_board[i-6+3];				
			}
			    else if(operators_array[i]== '-')
			    {										
				column_results[i-6] = main_board[i-6] - main_board[i-6+3];				
			...
			if(operators_array[i+3]== '*')
			{							
				column_results[i-6] *= main_board[i-6+6];				
			}
			    else if(operators_array[i+3]== '-')
			    {					
				    column_results[i-6] -= main_board[i-6+6];				
			    }
			        else
